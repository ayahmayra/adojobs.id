<x-layouts.dashboard>
    ...
    <div class="grid grid-cols-1 gap-6 mb-6 md:grid-cols-2 lg:grid-cols-5">
        ...
        <div class="p-6 bg-white rounded-lg shadow-sm">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    <p class="text-sm font-medium text-gray-600">Kandidat Favorit</p>
                    <p class="text-2xl font-bold text-amber-600">{{ number_format($stats['favorite_candidates'] ?? 0) }}</p>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 gap-6 lg:grid-cols-3">
        ...
        <div class="space-y-6">
            ...
            <div class="overflow-hidden bg-white rounded-lg shadow-sm">
                <div class="flex justify-between items-center px-6 py-4 border-b border-gray-200">
                    <h3 class="text-lg font-semibold text-gray-900">Kandidat Favorit</h3>
                    <a href="{{ route('employer.saved-candidates.index') }}" class="text-sm font-medium text-indigo-600 hover:text-indigo-800">
                        Lihat Semua →
                    </a>
                </div>
                <div class="divide-y divide-gray-200">
                    @forelse($favoriteCandidates as $savedCandidate)
                        @php
                            $seeker = $savedCandidate->seeker;
                            $user = $seeker->user;
                        @endphp
                        <a href="{{ route('seekers.show', $seeker) }}" class="block p-4 transition hover:bg-gray-50">
                                    ...
                                    <div class="flex justify-between items-start">
                                        <div class="flex-1 min-w-0">
                                            <div class="flex items-center gap-2">
                                                ...
                                                <svg class="flex-shrink-0 w-4 h-4 text-amber-500" fill="currentColor" viewBox="0 0 24 24">
                                                    <path d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z"/>
                                                </svg>
                                            </div>
                                            @if($seeker->current_job_title)
                                                <p class="mt-1 text-sm text-gray-600 truncate">{{ $seeker->current_job_title }}</p>
                                            @endif
                                            <p class="mt-1 text-xs text-gray-500">{{ $savedCandidate->created_at->diffForHumans() }}</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                    ...
                    @empty
                        <div class="p-8 text-center text-gray-600">
                            <svg class="mx-auto w-12 h-12 text-gray-400" fill="currentColor" viewBox="0 0 24 24">
                                <path d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z"/>
                            </svg>
                            <p class="mt-2 text-sm">Belum ada kandidat favorit.</p>
                            <p class="mt-1 text-xs text-gray-500">Tambahkan kandidat dari detail aplikasi untuk menyimpannya.</p>
                        </div>
                    @endforelse
                </div>
            </div>
